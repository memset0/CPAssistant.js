...
async function main() { // forked from https://github.com/LumaKernel/cf-fast-submit (ver: 2.8)
	...
	function checkRequirements() {
		if (!logged) {
			utils.error(`[${MODULE_NAME}] not logged in.`)
			return false
		}
		if (!$) {
			utils.error(`[${MODULE_NAME}] not found jQuery.`)
			return false
		}
		if (!ace) {
			utils.error(`[${MODULE_NAME}] not found ace.`)
			return false
		}
		return true
	}
	...
	async function tryToInit(first) {
		for (let i = 0; i < retryTimes; i++) {
			try {
				...
				removeForm()
				utils.error(`[${MODULE_NAME}] unexpected error has been occured.`)
				throw e
			}
			removeForm()
			await delay(retryInterval)
		}
		utils.error(`[${MODULE_NAME}] tried some times but failed.`)
	}
	function delay(ms) {
	...
	async function initAppendForm(first = true, doNotRegenerateOnSubmit = false) {
		...
		$form = $newForm
		// modified aera (begin)
		$('.problemindexholder').append($form)
		$form.css('border', '1px')
		// modified aera (end)
